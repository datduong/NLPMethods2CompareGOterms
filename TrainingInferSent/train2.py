...
class Train:
    ...
    def __init__(self, model, optimizer, dictionary, embeddings_index, config, best_acc):
        ...
        self.config = config
        self.criterion = nn.CrossEntropyLoss(weight=torch.FloatTensor([1.5, 0.75])) ## added weight
    ...
    def train_epochs(self, train_corpus, dev_corpus, start_epoch, n_epochs):
        ...
        for epoch in range(start_epoch, start_epoch + n_epochs):
            if not self.stop:
                ...
                if (epoch + 1) > 1 and 'sgd' in self.config.optimizer:
                ...
                if self.best_dev_acc < dev_acc:
                    ...
                    if 'sgd' in self.config.optimizer:
                        ...
                        print('Shrinking lr by : {0}. New lr = {1:.6f}'.format(self.config.lrshrink,
